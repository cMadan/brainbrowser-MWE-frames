...
*
* Copyright (C) 2011-2016 McGill University
*
...
$(function() {
  ...
  window.viewer = BrainBrowser.SurfaceViewer.start("brainbrowser", function(viewer) {
    ...
    viewer.addEventListener("loadcolormap", function(event) {
      var color_map    = event.color_map;
      var canvas       = color_map.createElement(0, 100);
    ...
    viewer.addEventListener("updateintensitydata", function(event) {
      ...
      var values = Array.prototype.slice.call(event.intensity_data.values);
    ...
    $("#axes-controls").change(function() {
      ...
      viewer.drawAxes(150, {name: axes_name, complete: true});
    ...
    $("#annotation-save").click(function() {
      ...
      if (BrainBrowser.utils.isNumeric(vertex_num)) {
        annotation = viewer.annotations.get(vertex_num, {
          model_name: picked_object.userData.model_name
        });
    ...
    $("#examples").click(function(e) {
      ...
      var name = e.target.href.replace(/^[^#]+#/, "");
          ...
          viewer.loadModelFromURL('/models/mouse_brain_outline.obj', {
            ...
            complete: function() {
              ...
              viewer.setTransparency(0.5, {
                shape_name: "mouse_brain_outline.obj_1"
              });
            },
            cancel: defaultCancelOptions(current_request)
          });
      ...
      history.replaceState({}, document.title, e.target.href);

      $("#left-hem-visible, #right-hem-visible").prop('checked', true);
      $("#hem-superior").prop('checked', true);
      $("#fix-range, #clamp-range, #flip-range").prop('checked', false);
      $("#threedee, #wireframe").prop('checked', false);
      $("#autorotateX, #autorotateY, #autorotateZ, #gridX, #gridY, #gridZ, #axes-controls").prop('checked', false);
    });
    ...
    // If several color maps are loaded to be blended, create
    ...
    function loadFirstModel() {
      var link = $("a.example[href=" + window.location.hash.replace(/^[^#]+/, "") + "]");
